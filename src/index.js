...
const { processItems } = require("./models.js");
...
async function refresh(
  options = { sync: sync = false, create: create = false },
) {
  if (options.sync) {
    await todoist.sync();
  }
  ...
  if (
    options.create && !projects.some((p) => p.name === state.currentProjectName)
  ) {
    await todoist.projects.add({ name: state.currentProjectName });
  }

  const currentProject =
    todoist.projects.get().find((p) => p.name === state.currentProjectName) ||
    projects[0];
  ...
  if (state.currentProjectName == "Today") {
    ...
    state.items = processItems(
      items.filter((i) => i.project_id === currentProject.id),
    );
  }
...
async function onCreate(direction) {
  ...
  const nextItem = state
    .items[nextIndex < 0 ? 0 : nextIndex > maxIndex ? maxIndex : nextIndex] ||
    { parent_id: null };
...
async function onUnindent() {
  ...
  const patch = {
    ...
    project_id: parentItem.parent_id === null
...
async function onProjectArchive() {
  if (!todoist.state.user.is_premium) {
    return setErrorMessage(
      "Project archiving is only available for premium users :/",
    );
  }
...
module.exports = (plugin) => {
  ...
  _command("TodoistEval", pcall((input) => eval(input)), {
    sync: false,
    nargs: 1,
  });
  ...
  _function("Todoist__onChangeContent", pcall(onChangeContent), {
    sync: false,
  });
  _function("Todoist__onChangeDate", pcall(onChangeDate), { sync: false });
  _function("Todoist__onChangePriority", pcall(onChangePriority), {
    sync: false,
  });
  _function("Todoist__onRefresh", pcall(onRefresh), { sync: false });
  _function("Todoist__onProjectArchive", pcall(onProjectArchive), {
    sync: false,
  });
  _function("Todoist__onProjectDelete", pcall(onProjectDelete), {
    sync: false,
  });
  _function("Todoist__onProjectChangeColor", pcall(onProjectChangeColor), {
    sync: false,
  });
  _function("Todoist__onProjectChangeName", pcall(onProjectChangeName), {
    sync: false,
  });

  _function("Todoist__listProjects", pcallSync(listProjects), { sync: true });
  _function("Todoist__completeProjects", pcallSync(completeProjects), {
    sync: true,
  });
  ...
  _command(
    ...
    pcall(() =>
      showWarningMessage(
        'TodoistInit deprecated. Use `:Todoist [project name]` (default "Inbox")',
      )
    ),
    { sync: false },
  );
...
process.on(
  "uncaughtException",
  (error, origin) => dumpError({ error, origin }),
);
process.on(
  "unhandledRejection",
  (error, promise) => dumpError({ error, promise }),
);
