...
function renderHeader(state) {
  ...
  const titleHl = currentProject.color === 48 && currentProjectName === "Inbox"
    ? "todoistTitle"
  ...
  const otherMessage = state.isLoading
    ? [[{ hl: "todoistMessage", text: "Loading…" }]]
    : [];

  const emptyMessage = state.items.length === 0 && !state.isLoading
...
function renderIndent(state, i) {
  return {
    hl: "Normal",
    text: " ".repeat(i.depth * (state.options.icons.checked.length - 1)),
  };
}
...
function renderCheckbox(state, i) {
  ...
  const text = i.loading
    ? state.options.icons.loading
    : i.error
    ? state.options.icons.error
    : i.checked
    ? state.options.icons.checked
